...
| | |
|---|---|
| TalaDB | v0.10.1 Node (vector re-run); v0.9.4 browser release build |
...
| Date | Node vector re-run 2026-07-25; browser re-run 2026-07-13 |
...
The vector rows below were re-measured on **0.10.1** (2026-07-25), which adds the decoded-vector cache. Document-write and query-latency rows retain the established v0.9.0 Node baseline — a 0.10.1 re-run reproduced them within run-to-run noise (25→25 µs point reads, 444→431 ms unindexed scan), so they are not disturbed.

## Node.js — vector search (384-dim, cosine, top-10)

The default (flat) index is exact k-NN over all vectors. Since **0.10.1** a decoded-vector cache keeps the vectors in memory after the first query, so repeated searches no longer re-read and re-decode the whole vector table from storage — the win grows with collection size, where that storage read dominates:

| Collection size | `findNearest` (0.10.1) | v0.9.0 | v0.8.x |
|---|---|---|---|
| 1,000 vectors | **2.9 ms** | 2.5 ms | 4.0 ms |
| 10,000 vectors | **18 ms** | 18 ms | 40 ms |
| 50,000 vectors | **86 ms** | 93 ms | 188 ms |
| 100,000 vectors | **176 ms** | 198 ms | 369 ms |

At 1k–10k vectors the storage read was already small, so the numbers are unchanged within run-to-run noise; the cache pays off at scale (≈11% at 100k) and — most of all — on **filtered** search, where avoiding the full scan is worth the most:

| Operation | Detail | Result |
|---|---|---|
| `findNearest` + filter, 100k vectors | indexed pre-filter `locale: "en"` (10%), then rank | **199 ms** *(was 346 ms — ~42% faster)* |
| Vector ingest, 100k vectors | `insertMany` with a live vector index | **~3.7k docs/s** |

Filtered vector search is the on-device RAG path — the metadata pre-filter narrows the candidate set, then only matching vectors are scored. With the cache, that query dropped from 346 ms to **199 ms** at 100k vectors.
